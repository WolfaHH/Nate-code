...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const projectIcons = useClientSettings((settings) => settings.projectIcons);
  const customProjectIconSrc = projectIcons?.[String(project.id)];
  ...
  const handleProjectButtonContextMenu = useCallback(
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        const hasCustomIcon = Boolean(projectIcons?.[String(project.id)]);
        actionHandlers.set("set-icon", () => {
          iconInputRef.current?.click();
        });
        actionHandlers.set("remove-icon", () => {
          const next = { ...(projectIcons ?? {}) };
          delete next[String(project.id)];
          updateSettings({ projectIcons: next });
        });
        const iconMenuItems: ContextMenuItem<string>[] = [
          { id: "set-icon", label: "Set custom icon…" },
          ...(hasCustomIcon ? [{ id: "remove-icon", label: "Remove custom icon" }] : []),
        ];
      ...
      <div className="group/project-header relative">
        ...
        >
          <ProjectFavicon
            ...
            overrideSrc={customProjectIconSrc}
          />
          <span className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <span className="flex shrink-0 items-center gap-1.5">
                ...
                <TooltipTrigger
                  render={
                    <span
                      ...
                      className={`inline-flex size-2.5 items-center justify-center ${projectStatus.colorClass}`}
                    >
                      <span
            ...
            <ChevronRightIcon
              className={`size-3.5 text-muted-foreground/60 opacity-0 transition-all duration-150 group-hover/project-header:opacity-100 ${
        ...
        <input
          ...
          onChange={(event) => {
            ...
            reader.onload = () => {
              ...
              updateSettings({
                projectIcons: { ...(projectIcons ?? {}), [String(project.id)]: reader.result },
              });
            };
...
const SidebarProjectsContent = memo(function SidebarProjectsContent(
  ...
  const createScratchChat = useNewScratchChat();
  // The "Chats" (projectless) project is pinned in its own section, not mixed
  // into the regular Projects list.
  const chatsProject = useMemo(
    () => sortedProjects.find((project) => isScratchChatsProject(project)) ?? null,
    [sortedProjects],
  );
  const displayProjects = useMemo(
    () => sortedProjects.filter((project) => !isScratchChatsProject(project)),
    [sortedProjects],
  );
      ...
      <SidebarGroup className="px-2 py-2">
        ...
        >
          <SidebarMenu ref={attachProjectListAutoAnimateRef}>
            <SortableContext
              items={displayProjects.map((project) => project.projectKey)}
          ...
          <SidebarMenu>
            <SidebarMenuItem>
              <SidebarMenuButton
                size="sm"
                className="gap-2 px-2 py-1.5 text-muted-foreground/70 hover:bg-accent hover:text-foreground"
                data-testid="sidebar-new-scratch-chat"
                onClick={() => void createScratchChat()}
              >
                <SquarePenIcon className="size-3.5" />
                <span className="flex-1 truncate text-left text-xs">New chat (no project)</span>
              </SidebarMenuButton>
            </SidebarMenuItem>
          </SidebarMenu>
        )}
...
export default function Sidebar() {
  ...
  // Global agent notification sounds (completed / awaiting) across all threads —
  // mounted here since the sidebar is always rendered alongside the chat.
  useAgentSounds(sidebarThreads);
  const agentSoundsVolume = useClientSettings((s) => s.agentSoundsVolume);
  useEffect(() => {
    setAgentSoundsVolume(agentSoundsVolume / 100);
  }, [agentSoundsVolume]);
  ...
  useEffect(() => {
    // Drive the macOS vibrancy frost intensity: lowering opacity makes the app's
    // surfaces translucent so the native blurred backdrop shows through, while text
    // stays fully crisp. (Native `setOpacity` is intentionally NOT used — it fades the
    // text too and is a no-op on this macOS setup anyway.)
    document.documentElement.style.setProperty("--surface-opacity", `${windowOpacity}%`);
  }, [windowOpacity]);
  ...
  const handleProjectDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      // Dragging implies a manual order. When the list is auto-sorted, seed the
      // manual baseline from the on-screen order so nothing jumps, then flip the
      // sort to manual and persist the move.
      const baseOrder = isManualProjectSorting
        ? orderedProjects.map(getProjectOrderKey)
        : sortedProjects.flatMap((project) =>
            project.memberProjects.map((member) => member.physicalProjectKey),
          );
      reorderProjects(baseOrder, activeMemberKeys, overMemberKeys);
